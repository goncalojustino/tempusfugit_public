...
export default function AdminMaintenance({ token }){
  ...
  async function load(){
    try{
      const r=await fetch(`${API}/admin/maintenance`,{ headers:{'Authorization':'Bearer '+token} })
  ...
  async function add(){
    ...
    const r=await fetch(`${API}/admin/maintenance/add`,{
  ...
  async function del(id){
    ...
    const r=await fetch(`${API}/admin/maintenance/delete`,{
      method:'POST', headers:{'Content-Type':'application/json','Authorization':'Bearer '+token},
      ...
      <table>
        ...
        <tbody>
          {rows.map(r=> {
            ...
            // Heuristic: rows with same resource, time-of-day and duration appearing multiple times → weekly series
              ...
              <td>
                <div style={{display:'flex',gap:6}}>
                  ...
                  <button title="Stop this series from here" onClick={async()=>{
                    ...
                    const res = await fetch(`${API}/admin/maintenance/stop`,{
                      method:'POST', headers:{'Content-Type':'application/json','Authorization':'Bearer '+token},
