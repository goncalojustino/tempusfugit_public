...
export default function MaintenanceMobile({ token }){
  const [rows,setRows]=useState([])
  const [form,setForm]=useState({ resources:[], date:'', startTime:'', endTime:'', repeat:'once', reason:'' })
  useEffect(()=>{ load() },[])
  async function load(){ try{ const r=await fetch(`${API}/admin/maintenance`,{ headers:{'Authorization':'Bearer '+token} }); const j=await r.json(); setRows(Array.isArray(j)?j:[]) }catch{ setRows([]) } }
  ...
  async function add(){
    ...
    const r=await fetch(`${API}/admin/maintenance/add`,{ method:'POST', headers:{'Content-Type':'application/json','Authorization':'Bearer '+token}, body: JSON.stringify(b) })
    const j=await r.json(); if(!r.ok) return alert(j.error||'ERROR'); setForm({ resources:[], date:'', startTime:'', endTime:'', repeat:'once', reason:'' }); load() }
  async function del(id){ if(!confirm('Delete maintenance?')) return; await fetch(`${API}/admin/maintenance/delete`,{ method:'POST', headers:{'Content-Type':'application/json','Authorization':'Bearer '+token}, body: JSON.stringify({id}) }); load() }
  return (
    <div>
      <div className="card" style={{padding:12,display:'grid',gap:8,marginBottom:12}}>
        <div style={{fontWeight:600}}>Add maintenance</div>
