...
export function shiftLisbonMidnight(baseInstant, deltaDays){
  const parts = instantToLisbonDateTime(baseInstant)
  const shiftedParts = shiftLisbonDateParts(parts, deltaDays)
  return lisbonDateTimeToInstant({ ...shiftedParts, hour: 0, minute: 0, second: 0 })
}
...
export function formatLisbon(date, options){
  const key = JSON.stringify(options || {})
  let formatter = dateFormatterCache.get(key)
  if (!formatter){
    formatter = new Intl.DateTimeFormat('en-GB', { timeZone: LISBON_TZ, ...options })
...
export function formatLisbonDateTime(date){
  return `${formatLisbon(date, { year: 'numeric', month: '2-digit', day: '2-digit' })} ${formatLisbon(date, { hour: '2-digit', minute: '2-digit' })}`
}

export function lisbonISOFromParts(parts){
  return lisbonDateTimeToInstant(parts).toISOString()
}

export function lisbonISOFromDate(date){
  return lisbonMidnightInstant(date).toISOString()
}
